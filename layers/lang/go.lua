...
function L.complete(_)
	lsp.enable("gopls", { settings = { gopls = { gofumpt = true, buildFlags = { "-tags=wireinject" } } } })
end
...
function L.generate_getters()
	...
	vim.cmd(
		string.format(
			[['<,'>s/\(\w\+\)\s\+\([a-zA-Z.0-9]\+\)/func (%s %s) \u\1() \2 { return %s.\1 }/g]],
			receiver,
			receiver_type,
			receiver
		)
...
function L.generate_setters()
	...
	vim.cmd(
		string.format(
			[['<,'>s/\(\w\+\)\s\+\([a-zA-Z.0-9]\+\)/func (%s %s) Set\u\1(value \2) { %s.\1 = value }/g]],
			receiver,
			receiver_type,
			receiver
		)
