...
local function register_implementation(s, filetype, commandName, implementation)
    if s.commands[commandName] == nil then s.commands[commandName] = {} end
...
local function show_unimplemented_commands(s)
    ...
    for command, implementation in pairs(s.commands) do
        if implementation['*'] == '' then print('-', command) end
    end
...
function L.command_shortcut(commandName) return '<cmd>' .. commandName .. '<CR>' end
